...
function M.on_show()
    M.state = "break"
    M.remaining_seconds = M.break_time_mins * 60
    M.time = tostring(M.break_time_mins) .. ":" .. "00"
...
function M.draw()
    if M.state == "break" then
        ...
        gdisplay.write({ gdisplay.CENTER, 0 }, "Break time!")
    ...
    elseif M.state == "done" then
        gdisplay.setfont(gdisplay.FONT_DEFAULT)
        gdisplay.write({ gdisplay.CENTER, 0 }, "Yay!")
        gdisplay.write({ gdisplay.CENTER, 24 }, "Break period")
